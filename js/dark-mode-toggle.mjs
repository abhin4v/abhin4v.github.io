...
export class DarkModeToggle extends HTMLElement {
  ...
  constructor() {
    ...
    this._darkCSS = new Set();
    this._lightCSS = new Set();
  ...
  _initializeDOM() {
    ...
    this._lightCSS.add(...this._getLightCSS());
    this._darkCSS.add(...this._getDarkCSS());
  ...
  _getDarkCSS() {
    return doc.querySelectorAll(`${LINK_REL_STYLESHEET}[${MEDIA}*=${PREFERS_COLOR_SCHEME}][${MEDIA}*="${DARK}"]`);
  }

  _getLightCSS() {
    return doc.querySelectorAll(`${LINK_REL_STYLESHEET}[${MEDIA}*=${PREFERS_COLOR_SCHEME}][${MEDIA}*="${LIGHT}"],${LINK_REL_STYLESHEET}[${MEDIA}*=${PREFERS_COLOR_SCHEME}][${MEDIA}*="${NO_PREFERENCE}"]`);
  }
  ...
  _updateMode() {
    const lcss = this._getLightCSS();
    if (lcss.length != 0) {
      this._lightCSS.add(...lcss);
    }
    const dcss = this._getDarkCSS();
    if (dcss.length != 0) {
      this._darkCSS.add(...dcss);
    }
